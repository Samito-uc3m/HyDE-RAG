...
def run_correlation_filter(query_str: str, output_language: str, retrieved_docs: List[DocListResponse], llm) -> ChatResponse:
    ...
    system_content = (
        ...
        f"summarize key findings, highlight similarities, and note any gaps or differences. Respond clearly and concisely in {output_language}."
    )
    ...
    user_instructions = (
        ...
        "2. Follow the introductory paragraph with a structured list for the top documents in the corresponding language:\n"
        "   - Title: '<document_title>' (Similarity: <similarity_score>): <brief_summary>\n"
    ...
    return llm.chat(messages=messages).raw.choices[0].message.content.strip()
